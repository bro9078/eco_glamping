...
const translations = {
  ru: {
    ...
    contact_title: "Забронируйте отдых у озера",
    contact_lead: "Оставьте заявку, и мы свяжемся с вами, чтобы подобрать домик и даты — или напишите нам напрямую.",
    ...
    form_title: "Оставить заявку на бронь",
    ...
    form_dates_label: "Желаемые даты",
    form_dates_placeholder: "напр. 12–14 августа",
    form_message_label: "Сообщение",
    form_submit: "Отправить заявку",
    form_success: "Спасибо! Ваша заявка отправлена, мы свяжемся с вами в ближайшее время.",
  ...
  kk: {
    ...
    contact_title: "Көл жағасында демалысты брондаңыз",
    contact_lead: "Өтінім қалдырыңыз, біз сізге үй мен күндерді таңдау үшін хабарласамыз — немесе бізге тікелей жазыңыз.",
    ...
    form_title: "Брондауға өтінім қалдыру",
    form_name_label: "Атыңыз",
    form_name_error: "Атыңызды енгізіңіз",
    form_phone_label: "Телефон",
    form_phone_error: "Дұрыс телефон нөмірін енгізіңіз",
    form_dates_label: "Қалаған күндер",
    form_dates_placeholder: "мыс. 12–14 тамыз",
    form_message_label: "Хабарлама",
    form_submit: "Өтінім жіберу",
    form_success: "Рахмет! Сіздің өтініміңіз жіберілді, біз жақын арада сізбен хабарласамыз.",
  ...
  en: {
    ...
    contact_title: "Book your stay by the lake",
    contact_lead: "Leave a request and we'll get in touch to help pick a cabin and dates — or message us directly.",
    ...
    form_title: "Leave a Booking Request",
    form_name_label: "Your Name",
    form_name_error: "Please enter your name",
    form_phone_label: "Phone",
    form_phone_error: "Please enter a valid phone number",
    form_dates_label: "Preferred Dates",
    form_dates_placeholder: "e.g. Aug 12–14",
    form_message_label: "Message",
    form_submit: "Send Request",
    form_success: "Thank you! Your request has been sent, we'll be in touch shortly.",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const revealTargets = document.querySelectorAll(
    ".about-text, .about-media, .acc-card, .amenity, .gallery-item, .why-list li, .why-media, .faq-item, .contact-info, .contact-form, .banya-block, .section-title, .section-sub"
  );
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (nameInput.value.trim().length < 2) {
      setInvalid(nameRow, true);
      valid = false;
    } else {
      setInvalid(nameRow, false);
    }

    // Phone validation (simple international format check)
    const phoneRow = phoneInput.closest(".form-row");
    const phonePattern = /^[+]?[0-9\s()-]{7,20}$/;
    if (!phonePattern.test(phoneInput.value.trim())) {
      setInvalid(phoneRow, true);
      valid = false;
    } else {
      setInvalid(phoneRow, false);
    }

    if (!valid) return;

    // NOTE: No backend is connected yet — this simply confirms submission
    // to the user. Replace with a real endpoint / integration (email,
    // CRM, WhatsApp API, etc.) when going live.
